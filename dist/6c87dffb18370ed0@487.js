...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["visit-sequences@1.csv", new URL("./PaxHeader", import.meta.url)],
  ]);
  main.builtin(
    "FileAttachment",
...
`;
  ...
  main
    .variable(observer("csv"))
    .define(
      "csv",
      ["d3", "FileAttachment"],
      async function (d3, FileAttachment) {
        return d3.csvParseRows(
          await FileAttachment("visit-sequences@1.csv").text()
        );
      }
    );
  main
    .variable(observer("data"))
    .define("data", ["buildHierarchy", "csv"], function (buildHierarchy, csv) {
      return buildHierarchy(csv);
    });
  main
    .variable(observer("partition"))
    .define("partition", ["d3", "radius"], function (d3, radius) {
      return (data) =>
  ...
  main
    ...
    .define("mousearc", ["d3", "radius"], function (d3, radius) {
      return d3
        .arc()
        .startAngle((d) => d.x0)
        .endAngle((d) => d.x1)
  ...
  main
    ...
    .define("buildHierarchy", function () {
  ...
  main
    ...
    .define("breadcrumbWidth", function () {
      return 75;
    });
  main
    .variable(observer("breadcrumbHeight"))
    .define("breadcrumbHeight", function () {
      return 30;
    });
  main
